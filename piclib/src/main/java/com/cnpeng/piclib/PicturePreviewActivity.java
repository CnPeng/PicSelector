...
import com.cnpeng.piclib.adapter.SimpleFragmentAdapter;
            ...
            public void onClick(View view) {
                if (images != null && images.size() > 0) {
                    ...
                    if (config.maxSize < image.getFileLength()) {
                        ...
                        String str = "单文件大小请勿超过" + ExternalStorageUtils.converSize((config.maxSize));
        ...
        if (id == R.id.id_ll_ok) {
            ...
            if (config.enableCrop && pictureType.startsWith(PictureConfig.IMAGE)) {
                if (config.selectionMode == PictureConfig.SINGLE) {
                    ...
                    for (LocalMedia media : selectImages) {
                        cuts.add(media.getOriginalPath());
                    }
                    startCrop(cuts);
                }
            } else {
