...
public class CaptureButton extends View {
    public static final String MODE_TAKE_PHOTO           = "takePhoto";     //单纯拍照
    public static final String MODE_TAKE_PHOTO_AND_AUDIO = "takePhotoAndAudio";     //拍照+录像
    public static final String MODE_RECORD_AUDIO         = "recordAudio";     //录像
    ...
    private String            captureButtonMode;   //功能按钮的模式，拍照、拍照+录像，取值 MODE_TAKE_PHOTO、MODE_TAKE_PHOTO_AND_AUDIO
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                if (STATE_SELECTED == STATE_LESSNESS) {     //空闲时
                    if (event.getY() > btn_return_Y - 37 && event.getY() < btn_return_Y + 10 && event.getX() >
                        ...
                        if (MODE_TAKE_PHOTO_AND_AUDIO.equals(captureButtonMode)) {
                            postCheckForLongTouch();  //长按了按钮。短按拍照时不会走这里，会直接走到UP。加模式判断是为了在发布评论时屏蔽长按录像操作
                        } else if (MODE_RECORD_AUDIO.equals(captureButtonMode)) {
                            post(longPressRunnable);
                        } else {    //171115 增加判断，处理创建评论时只能拍照不能拍视频的情况
                    ...
                    if (event.getY() > btn_center_Y - btn_result_radius && event.getY() < btn_center_Y +
                        ...
                        if (mCaptureListener != null) {     //拍照预览或者视频预览时点击了返回键
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (STATE_SELECTED == STATE_READYQUIT) {
                    ...
                    Log.e("CaptureButton", "record_anim.getCurrentPlayTime==" + record_anim.getCurrentPlayTime());
                    ...
                    if (btn_outside_radius == btn_after_outside_radius && btn_inside_radius ==
                        ...
                        startAnimation(btn_after_outside_radius, btn_before_outside_radius, btn_after_inside_radius,
    ...
    private class RecordRunnable implements Runnable {
        ...
        public void run() {
            ...
            //注意，此处使用动画监听处理动画执行完毕的事件。如果用户在 "开发者选项" 中将"动画程序时长缩放" 关闭了，就会导致 onAnimationEnd
            // 无法正常监测动画执行情况，出现动画瞬时开启瞬时关闭的问题，然后就会导致 JCameraView 中 stopRecord() 中 mediaRecorder.stop()
            //位置崩溃的情况，崩溃信息：RuntimeExcepiton: stop fail
    ...
    public void setCaptureButtonMode(String mode) {
        if (CaptureButton.MODE_TAKE_PHOTO.equals(mode)) {
            this.captureButtonMode = CaptureButton.MODE_TAKE_PHOTO;
        } else if (MODE_RECORD_AUDIO.equals(mode)) {
            this.captureButtonMode = CaptureButton.MODE_RECORD_AUDIO;
        } else {
            this.captureButtonMode = CaptureButton.MODE_TAKE_PHOTO_AND_AUDIO;
        }
